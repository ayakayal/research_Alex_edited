...
def update_target_graph(from_scope, to_scope):
    ...
    return op_holder
class DQN_Agent:
    def __init__(self, env,beta_exp):
        self.env = env
                ...
                while not done and episode_steps< self.env.max_episode_steps:
                    ...
                    if (total_steps < pre_train_steps) and train_model:
                        ...
                        for action in range(self.env.actions_n):
                            if tuple ([observation,action]) not in seq:
                                seq_temp= seq + [tuple ([observation,action])]
                            else:
                                seq_temp=seq

                            print('seq temp ',seq_temp)
                            Bonuses[action] = self.reward_calc(self.intrinsic_reward, self.seq_freq[tuple(seq_temp)], episode_steps, alg='MBIE-EB')
                        print('dictionary ',self.seq_freq)
                    ...
                    if not train_model and np.random.rand(1) < 0.1:
                        action = np.random.randint(0,action_space_size)
                    if tuple([observation,action]) not in seq:
                        seq.append(tuple ([observation,action]))
