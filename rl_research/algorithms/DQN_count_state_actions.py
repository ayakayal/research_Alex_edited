...
class DQNCountStateAction(DQN_Agent):
    ...
    def select_action(self, state, policy_net):
        ...
        #print('self.env.actions_n ',self.env.actions_n)
        for action in range(self.env.actions_n):
            #print('action index of bonus ',action)
            Bonuses[action] = self.reward_calc(self.intrinsic_reward, self.state_action_freq[tuple ([state,action])], self.timestep, alg='MBIE-EB')
            #print('bonuses ',Bonuses)        
        #print('bonuses after converting to tensor',tf.convert_to_tensor(Bonuses,dtype='float32'))
        #print('policy ',policy_net(np.atleast_2d(np.atleast_2d(state).astype('float32'))))
        #print('sum policy and bonus ',policy_net(np.atleast_2d(np.atleast_2d(state).astype('float32')))+tf.convert_to_tensor(Bonuses,dtype='float32'))
        action_idx=np.argmax(policy_net(np.atleast_2d(np.atleast_2d(state).astype('float32')))+tf.convert_to_tensor(Bonuses,dtype='float32'))
        #print('action chosen ',action_idx)
        self.state_action_freq[tuple ([state,action_idx])] += 1
